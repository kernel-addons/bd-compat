...
var path = BDCompatNative.executeJS(`require("path")`);
...
class DataStore {
	...
	static saveAddonState(type1, state = {
		...
		try {
			fs.writeFileSync(path.resolve(this.dataFolder, `${type1}States.json`), JSON.stringify(state, null, "\t"));
		} catch (error) {
	...
	static saveData(pluginName1, data) {
		try {
			fs.writeFileSync(path.resolve(this.dataFolder, `${pluginName1}.json`), JSON.stringify(data, null, "\t"), "utf8");
		} catch (error) {
	...
	static deletePluginData(pluginName4, key2) {
		...
		delete this.pluginData[pluginName4].settings[key2];
 ...
 * @returns {[(collector = _ => _) => JSON.Element, Api]}
 */
...
class PluginsManager {
	...
	static watchAddons() {
		...
		this.watcher = fs.watch(this.folder, {
			persistent: false
		}, (eventType, filename) => {
...
var index = new class BDCompat {
	start() {
		Webpack1.once(Events.LOADED, this.onStart.bind(this));
	}
	onStart() {
	...
	patchSettingsView() {
		...
		Patcher.after("BDCompatSettings", SettingsView.prototype, "getPredicateSections", (_, __, res) => {
			if (!Array.isArray(res) || !res.some((e) => {
					...
					return (s === null || s === void 0 ? void 0 : s.id) === "kernel-settings";
				})) return;
